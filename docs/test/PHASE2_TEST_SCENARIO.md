...
> **참고**: 동시접속 감지 시 백엔드에서 토큰을 미리 생성하여 임시 저장합니다.
> "기존 세션 종료하고 로그인" 클릭 시 임시 저장된 토큰으로 즉시 로그인이 완료됩니다.
> (Google 로그인 화면으로 다시 이동하지 않음)

#### Step 3: 로그인 성공 확인
1. 브라우저 B에서 **즉시** 로그인 성공 화면 표시
2. 2초 후 대시보드 화면으로 자동 이동
...
- **Google 로그인 화면 없이 즉시 로그인 완료** (UX 개선)
- 새 세션 생성 및 토큰 발급
- 브라우저 B에서 대시보드 접근 가능
...
- [ ] 토큰이 임시 저장됨 (pending login store)

### ✅ 기존 세션 종료 후 재로그인
- [ ] "기존 세션 종료하고 로그인" 클릭 시 complete-force-login API 호출
- [ ] 기존 세션의 revoked_yn = 'Y'로 변경
- [ ] Google 로그인 화면 없이 즉시 로그인 완료
- [ ] 임시 저장된 토큰으로 새 세션 생성
- [ ] 대시보드로 정상 이동
...
- [ ] `/auth/complete-force-login`: 기존 세션 폐기 후 즉시 로그인 완료
...
### 재로그인이 실패하는 경우
1. **Pending Login 만료 확인**:
   - 동시접속 모달 표시 후 5분이 지나면 임시 저장된 토큰이 만료됩니다
   - 에러 메시지: "로그인 정보가 만료되었습니다. 다시 로그인해 주세요."
   - 이 경우 처음부터 다시 Google 로그인을 진행해야 합니다
2. **세션 폐기 확인**:
   - DB에서 revoked_yn = 'Y'인지 확인
3. **Network 탭 확인**:
   - `/api/v1/auth/complete-force-login` 호출이 200으로 응답했는지 확인
   - 401 에러 시 pending login이 만료되었거나 없는 경우

...
🔄 새로운 Google 로그인 플로우 시작
```

> **참고**: Google OAuth authorization code는 일회용이므로, 기존 세션 폐기 후 새로운 Google 로그인 플로우가 시작됩니다.

#### Step 3: Google 로그인 화면으로 리다이렉트
1. 버튼 클릭 후 자동으로 Google 로그인 화면으로 이동
2. 이미 Google에 로그인되어 있으면 자동으로 계정 선택 화면 표시
3. 같은 계정을 다시 선택
...
- Google 로그인 화면으로 리다이렉트됨
- 새 authorization code로 로그인하여 새 세션 생성
- 브라우저 B에서 대시보드 접근 가능
...
2. **Google 로그인 화면 리다이렉트 확인**:
   - "기존 세션 종료하고 로그인" 클릭 후 Google 로그인 화면으로 이동해야 합니다
   - 같은 페이지에서 에러가 발생하면 프론트엔드 코드 확인 필요
3. **Network 탭 확인**:
   - `/api/v1/auth/revoke-session` 호출이 200으로 응답했는지 확인
   - 이후 Google OAuth URL로 리다이렉트되는지 확인

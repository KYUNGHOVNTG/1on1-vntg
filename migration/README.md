...
이 `migration/` 폴더에 있는 `.sql` 파일들은 **프로젝트 초기에 참고용으로 작성된 레거시 문서**입니다.
실제 DB 변경에는 사용되지 않으며, 초기 스키마 구조를 이해하기 위한 참고 자료로만 활용됩니다.

**새로운 SQL 파일을 이 폴더에 추가하지 마세요.** 모든 DB 변경은 반드시 `alembic revision`으로 생성하세요.
